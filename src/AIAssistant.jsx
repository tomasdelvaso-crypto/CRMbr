...
import { X, Target, DollarSign, Brain, Send, Loader2, Bot, Sparkles, AlertCircle, Activity, BarChart3, Clock, CheckCircle, ChevronRight, Zap, AlertTriangle, Maximize2, Minimize2, RefreshCw, ChevronDown, ChevronUp, Check } from 'lucide-react';
        ...
        <div className="p-3 space-y-2">
          {alerts.slice(0,2).map((a,i) => (
            <div key={i} className={`text-sm p-3 rounded-lg border flex items-start ${ac(a.type)}`}>
              ...
              <div className="flex-1"><div className="font-semibold">{a.message}</div>{a.action&&<div className="text-sm opacity-90 mt-0.5">→ {a.action}</div>}</div>
            </div>
          ))}
        </div>
      )}
      {!opportunity && pipeline && (
        <div className="p-4">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="bg-white rounded-lg p-3 border"><DollarSign className="w-5 h-5 text-green-600 mb-1"/><div className="text-base font-bold">R$ {(pipeline.totalValue/1000000).toFixed(1)}M</div><div className="text-sm text-gray-500">Pipeline Total</div></div>
          </div>
          {pipeline.atRisk > 0 && <div className="mt-2 text-sm bg-red-50 text-red-700 p-3 rounded-lg">⚠️ {pipeline.atRisk} negócios em risco</div>}
        </div>
      )}
    </div>
  );
...
const PersistentActionCard = ({ activity, onMarkDone, onDiscard, onUseInChat }) => {
    ...
    <div className="border-l-4 border-indigo-400 bg-indigo-50 rounded-lg p-4 hover:shadow-md transition-all">
      ...
      {!mode && (
        <div className="mt-3 flex flex-wrap gap-2">
          ...
          {onUseInChat && <button onClick={() => onUseInChat(activity)} className="py-2.5 px-3 bg-purple-600 text-white rounded-lg text-sm font-bold hover:bg-purple-700"><Send className="w-4 h-4"/></button>}
...
const AIAssistant = ({ currentOpportunity, onOpportunityUpdate, currentUser, supabase }) => {
  ...
  const loadPendingFromDB = useCallback(async () => {
    ...
    try {
      const { data, error } = await supabase.from('activities').select('*')
        .eq('opportunity_id', currentOpportunity.id).eq('source', 'ai_generated')
        .eq('next_action_done', false).is('result', null).order('created_at', { ascending: true });
      if (!error) setPendingSuggestions(data || []);
  ...
  const saveSuggestionsToDB = async (actions) => {
    ...
    const records = actions.map(a => ({
      ...
      vendor: currentUser || 'Ventus',
      activity_type: 'ai_suggestion',
      description: a.title + ' — ' + a.description,
      result: null,
      stage_at_time: currentOpportunity.stage,
      methodology_code: null,
      ai_suggested_action: a.draft_content || null,
    ...
    try {
      await supabase.from('activities').insert(records);
    } catch (e) { console.error('Error saving suggestions:', e); }
  };

  const expireOldSuggestions = async () => {
    if (!supabase || !currentOpportunity) return;
    try {
      await supabase.from('activities').update({ next_action_done: true, result: 'expirado' })
        .eq('opportunity_id', currentOpportunity.id).eq('source', 'ai_generated')
        .eq('next_action_done', false).is('result', null);
    } catch (e) { console.error('Error expiring:', e); }
  };
  ...
  const generateActions = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/assistant', {
        ...
        body: JSON.stringify({ requestType: 'action_plan', opportunityData: currentOpportunity, vendorName: currentUser, pipelineData })
      });
      if (response.ok) {
        const data = await response.json();
        if (data.actionPlan?.actions?.length > 0) {
          await saveSuggestionsToDB(data.actionPlan.actions);
        }
        if (data.analysis) { setAnalysis(data.analysis); if (data.analysis.quickActions) setDynamicActions(data.analysis.quickActions); }
      }
      await loadPendingFromDB();
    } catch (e) { console.error('Error:', e); alert('Erro ao gerar ações'); }
  ...
  const loadPipelineData = async () => {
    if (!supabase) return;
    try { const { data, error } = await supabase.from('opportunities').select('*').order('value', { ascending: false }); if (!error && data) setPipelineData({ allOpportunities: data }); } catch (e) { console.error(e); }
  };
  ...
  const getUpdatedAnalysis = async () => {
    ...
    try {
      ...
      if (r.ok) { const d = await r.json(); if (d.analysis) { setAnalysis(d.analysis); if (d.analysis.quickActions) setDynamicActions(d.analysis.quickActions); } }
    } catch (e) { console.error(e); }
  };

  const handleUseInChat = (activity) => {
    const title = activity.next_action || '';
    const draft = activity.ai_suggested_action || activity.description || '';
    processMessage(`Me ajude a executar esta ação para ${currentOpportunity?.client || 'o cliente'}: ${title}. ${draft}`);
  };

  useEffect(() => { if (isOpen && supabase) loadPipelineData(); }, [isOpen]);
  useEffect(() => {
    if (currentOpportunity && isOpen) { getUpdatedAnalysis(); loadPendingFromDB(); }
    else { setPendingSuggestions([]); }
  ...
  const processMessage = async (text) => {
    ...
    try {
      ...
      if (d.analysis) { setAnalysis(d.analysis); if (d.analysis.quickActions) setDynamicActions(d.analysis.quickActions); }
    } catch (e) {
  ...
  const getContextualSuggestions = () => {
    ...
    const s = [];
    if ((currentOpportunity.scales?.dor?.score||0) < 5) s.push("🎯 Como posso fazer o cliente admitir sua dor?");
    if ((currentOpportunity.scales?.poder?.score||0) < 5) s.push("👤 Como acesso o verdadeiro tomador de decisão?");
    if (currentOpportunity.value > 100000) s.push("💰 O cliente diz que está muito caro, como manejo?");
    return s.slice(0,3);
  };

  const windowClasses = isMaximized ? 'fixed inset-4 z-50' : 'fixed bottom-24 right-6 w-[900px] h-[calc(100vh-140px)] max-h-[900px] z-50';
            ...
            <div className="flex justify-between items-center">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm opacity-90">{currentOpportunity ? `🎯 ${currentOpportunity.client} - ${currentOpportunity.name}` : '📊 Análise do Pipeline'}</p>
                </div>
              </div>
                ...
                <div className="border-b">
                  <div className="p-4 flex items-center justify-between bg-gradient-to-r from-indigo-50 to-purple-50 border-b border-indigo-200">
                    <div className="flex items-center">
                      ...
                      <span className="font-bold text-base text-indigo-800">Ações Sugeridas ({pendingSuggestions.length})</span>
                    </div>
                    <button onClick={generateActions} disabled={generating}
                      className="flex items-center px-3 py-1.5 bg-indigo-600 text-white rounded-lg text-sm font-semibold hover:bg-indigo-700 disabled:opacity-50">
                  ...
                  {generating && (
                    <div className="p-6 text-center bg-indigo-50">
                      ...
                      <p className="text-sm text-indigo-700">Analisando oportunidade e gerando ações personalizadas...</p>
                    </div>
                  )}
                  ...
                  {!generating && pendingSuggestions.length > 0 && (
                    <div className="p-4 space-y-3 bg-gradient-to-b from-indigo-50 to-white">
                      {pendingSuggestions.map(s => (
                        <PersistentActionCard key={s.id} activity={s}
                          onMarkDone={markDone} onDiscard={markDiscarded} onUseInChat={handleUseInChat} />
                      ))}
                    </div>
                  )}
                  ...
                  {!generating && pendingSuggestions.length === 0 && (
                    <div className="p-6 text-center bg-gray-50">
                      ...
                      <button onClick={generateActions} className="text-sm text-indigo-600 font-semibold hover:text-indigo-800">🤖 Gerar sugestões personalizadas →</button>
                    </div>
                  )}
                </div>
              )}
              ...
              {currentOpportunity && effectiveActions.length > 0 && (
                <div className="p-3 bg-gray-50 border-b flex-shrink-0">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-sm font-semibold text-gray-500">Ações rápidas</span>
                    <button onClick={() => setShowQuickActions(!showQuickActions)} className="text-sm text-gray-400 hover:text-gray-600">{showQuickActions?'ocultar':'mostrar'}</button>
                  </div>
                  {showQuickActions && (
                    <div className="grid grid-cols-3 gap-2">
                      {effectiveActions.slice(0,6).map((a,i) => (
                        <button key={i} onClick={() => processMessage(a.prompt)} disabled={isLoading}
                          className={`${a.color||'bg-gray-500'} text-white rounded-lg px-3 py-2.5 text-sm hover:opacity-90 flex flex-col items-center gap-1 disabled:opacity-50`} title={a.prompt}>
                          <span className="text-lg">{a.icon}</span><span className="font-medium">{a.label}</span>
                        </button>
                      ))}
                    </div>
                  )}
              ...
              <div className="flex-1 overflow-y-auto p-5 space-y-4 bg-gradient-to-b from-gray-50 to-white">
                  ...
                  <div className="bg-gradient-to-br from-purple-50 to-blue-50 p-6 rounded-xl">
                    ...
                    {currentOpportunity ? (
                      <>
                        ...
                        {analysis && (
                          <div className="text-sm bg-white rounded-lg p-3 mb-3 border border-purple-200">
                            <span className="font-semibold text-purple-700">Estado:</span>
                            <div className="mt-1">Saúde: {analysis.opportunity?.healthScore||'N/A'}/10 • Prob: {analysis.opportunity?.probability||'N/A'}% • {analysis.alerts?.length||0} alertas</div>
                          </div>
                        )}
                        {getContextualSuggestions().length > 0 && (
                          <div className="mt-3 space-y-2">
                            ...
                            {getContextualSuggestions().map((s,i) => (
                              <button key={i} onClick={() => processMessage(s)} className="w-full text-left text-sm bg-white hover:bg-purple-50 p-3 rounded-lg border border-purple-200">{s}</button>
                            ))}
                          </div>
                        )}
                      </>
                      ...
                      <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mt-2">
                        <p className="text-base text-yellow-800">⚠️ <strong>Selecione um cliente</strong> para análise completa</p>
                      </div>
                    )}
                  </div>
                )}
                ...
                {messages.map((msg, idx) => (
                  <div key={idx} className={`flex ${msg.role==='user'?'justify-end':'justify-start'}`}>
                    <div className={`max-w-[85%] ${msg.role==='user'?'bg-gradient-to-r from-purple-500 to-blue-500 text-white rounded-2xl rounded-tr-sm':'bg-white border border-gray-200 text-gray-800 rounded-2xl rounded-tl-sm shadow-sm'} p-4`}>
                      {msg.role==='assistant' && <div className="flex items-center gap-1 mb-1"><Bot className="w-4 h-4 text-purple-500"/><span className="text-sm text-purple-500 font-medium">Ventus Coach</span></div>}
                ...
                {isLoading && (
                  <div className="flex justify-start">
                    <div className="bg-white border rounded-2xl rounded-tl-sm p-4 shadow-sm flex items-center gap-2">
              ...
              <div className="p-4 border-t bg-white flex-shrink-0">
                <div className="flex gap-2">
                  ...
                  <button onClick={() => processMessage(input)} disabled={isLoading || !input.trim()}
                    className="bg-gradient-to-r from-purple-600 to-blue-600 text-white px-5 py-3 rounded-xl hover:shadow-lg disabled:opacity-50">
                ...
                <div className="mt-2 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    {currentOpportunity ? (
                      ...
                      {analysis?.opportunity && <span className="text-sm text-gray-400">| Saúde {analysis.opportunity.healthScore}/10</span>}</>
                    ) : <><div className="w-2 h-2 bg-yellow-500 rounded-full"/><span className="text-sm text-gray-500">Modo Pipeline</span></>}
                  </div>
